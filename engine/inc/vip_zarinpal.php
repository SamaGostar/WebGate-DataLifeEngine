...
echoheader("Offer","Vip ویژه");
 ...
 $id = intval($_GET['id']);
	...
	echo<<<HTML
	<!--</script>-->      
    <form method="post" style="margin:0; padding:0;" action="$PHP_SELF?mod=vip_zarinpal&action=move_save" name="formname" enctype="multipart/form-data">
...
echoheader("Offer","Vip ویژه");
	...
	$db->query("SELECT * FROM ".PREFIX."_vip_zarinpal where `show`='1'");
	$page_name="?mod=vip_zarinpal&order=page"; 
	$start=$_GET['start'];
	...
	$back = $eu - $limit; 
	$next = $eu + $limit; 
	$query2= $db->query("SELECT * FROM ".PREFIX."_vip_zarinpal where `show`='1' order by id desc");
	echo mysql_error();
	$nume= $db->num_rows( $query2 );
	$query_res = $db->query("SELECT * FROM ".PREFIX."_vip_zarinpal where `show`='1' order by id desc limit $eu, $limit");
	while ( $row = $db->get_row($query_res) ) {
		...
		<td class=\"list\" style=\"padding:4px;\"> {$row['date']}</td>
		<td class=\"list\" style=\"padding:4px;\"> <a href='$PHP_SELF?mod=vip_zarinpal&action=move&id=$row[userid]' title='انتقال دستی به گروه'> <img src=\"engine/skins/move.png\" align='absmiddle'> </a> </td>
		</tr>			
		";	
    ...
    $query = $db->query("SELECT * FROM ".PREFIX."_vip_zarinpal where `show`='1' order by id desc");		
...
//$db->query( "DELETE FROM " . PREFIX . "_vip_zarinpal WHERE `show`='1'" );
